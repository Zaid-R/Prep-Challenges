...
function capetalizeFirstLetter(input) {
    let firstLetter = input.charAt(0).toUpperCase();
    let inputWithoutFirstLetter = input.slice(1);
    return firstLetter + inputWithoutFirstLetter;
}

const objLat = (obj) => {
    let firstName = capetalizeFirstLetter(obj.firstName);
    let lastName = capetalizeFirstLetter(obj.lastName);
    return "my name is " + firstName + " " + lastName + " I am " + obj.age +
...
const cvFormatter = (arr) => {
    ...
    arr.forEach(obj => {
        if (obj.yearsOfExperience > 1) {
            ...
            newArray.push({
                fullName: obj.firstName + lastName,
                tech: obj.tech
            });
        }
    });
...
const classesAvg = (data) => {
    data.grades.forEach(obj => {
        obj.classes.forEach(clas => {
            let sum = 0;
            clas.classScores.forEach(score => {
                sum += score;
            });
            clas.avg = Math.floor(sum / clas.classScores.length);
        });
    });
    return data;
};
